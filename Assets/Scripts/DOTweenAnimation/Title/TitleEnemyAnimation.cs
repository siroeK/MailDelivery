using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using DG.Tweening;

/// <summary>
/// タイトルのエネミーアニメーション
/// </summary>
public class TitleEnemyAnimation : MonoBehaviour
{

    [Header("オブジェクト")]
    [SerializeField, Tooltip("胴体部分のオブジェクト")]
    private Transform body = null;
    [SerializeField, Tooltip("左足部分のオブジェクト")]
    private Transform leftFoot = null;
    [SerializeField, Tooltip("右足部分のオブジェクト")]
    private Transform rightFoot = null;

    [Header("移動させるアニメーション(DOLocalMoveZ)")]
    [SerializeField, Tooltip("左の目標地点")]
    private float leftTargetPos = 1.0f;
    [SerializeField, Tooltip("移動の演出時間")]
    private float moveProductionTime = 1.0f;
    [SerializeField, Tooltip("移動の変化の緩急（＝イージング）を指定")]
    private Ease moveEase = Ease.Unset;

    private Vector3 originalPos;    // 元の座標

    [Header("止まっている時のアニメーション(DOLocalMoveZ)")]
    [SerializeField, Tooltip("止まるところの座標")]
    private float stopPosZ = 0.15f;
    [SerializeField, Tooltip("止まっている時の演出時間")]
    private float stopProductionTime = 0.75f;
    [SerializeField, Tooltip("止まっている時の繰り返しタイプ")]
    private LoopType stopLoopType = LoopType.Restart;
    [SerializeField, Tooltip("止まっている時の変化の緩急（＝イージング）を指定")]
    private Ease stopEase = Ease.Unset;

    [SerializeField, Tooltip("止まる時の左足の座標")]
    private Vector3 leftFootPos = Vector3.zero;
    [SerializeField, Tooltip("止まる時の右足の座標")]
    private Vector3 rightFootPos = Vector3.zero;

    private Tween stopAnimationTween;           // 止まっている時アニメーションの返り値を用意

    [Header("移動アニメーション")]
    [Header("設定した複数の通過地点を通って移動させる(DOLocalPath)")]
    [SerializeField, Tooltip("右足移動の経由地点")]
    private Vector3[] rightMoveWayPoints = new Vector3[2];
    [SerializeField, Tooltip("左足移動の経由地点")]
    private Vector3[] leftMoveWayPoints = new Vector3[2];
    [SerializeField, Tooltip("移動の演出時間")]
    private float productionTime = 1.0f;
    [SerializeField, Tooltip("移動の曲線の種類")]
    private PathType movePathType = PathType.Linear;
    [SerializeField, Tooltip("移動の「最後のWayPoint→最初のWayPoint」が繋がるようするか")]
    private bool moveOption = true;
    [SerializeField, Tooltip("移動の繰り返しタイプ")]
    private LoopType moveLoopType = LoopType.Restart;
    [SerializeField, Tooltip("移動の変化の緩急（＝イージング）を指定")]
    private Ease ease = Ease.Unset;

    // Start is called before the first frame update
    void Start()
    {
        originalPos = this.transform.position;

        // アニメーション
        LeftMoveAnimation();
        StopAnimation();
        MoveAnimation();
    }

    /// <summary>
    /// DOTweenを使った左に移動時アニメーション
    /// </summary>
    private void LeftMoveAnimation()
    {
        // ローカル座標でZ移動
        this.transform.DOLocalMoveZ(
            leftTargetPos,              // 移動終了地点
            moveProductionTime          // 演出時間
        )
        .SetEase(moveEase)              // 変化の緩急（＝イージング）を指定、一定の速度で値が変化
        .OnStart(() =>                  // 実行開始時のコールバック
        {

        })
        .OnComplete(() =>               // 実行完了時のコールバック
        {
            // 元の座標に
            this.transform.position = originalPos;

            // 再度アニメーション
            LeftMoveAnimation();

        })
        .SetLink(gameObject);           // このGameObjectが削除された自動で止めるように紐付ける
    }

    /// <summary>
    /// DOTweenを使った止まっている時アニメーション
    /// </summary>
    private void StopAnimation()
    {
        // ローカル座標でZ移動
        this.stopAnimationTween = body.DOLocalMoveZ(
            stopPosZ,                   // 移動終了地点
            stopProductionTime          // 演出時間
        )
        .SetLoops(-1, stopLoopType)     // 繰り返し（-1 = ループ）、移動後、最初の位置から再び移動を開始
        .SetEase(stopEase)              // 変化の緩急（＝イージング）を指定、一定の速度で値が変化
        .OnStart(() =>                  // 実行開始時のコールバック
        {
            // 止まる時の左足、右足の座標を修正
            leftFoot.localPosition = leftFootPos;
            rightFoot.localPosition = rightFootPos;
        })
        .OnComplete(() =>               // 実行完了時のコールバック
        {

        })
        .SetLink(gameObject);           // このGameObjectが削除された自動で止めるように紐付ける
    }

    /// <summary>
    /// DOTweenを使った移動時アニメーション
    /// </summary>
    private void MoveAnimation()
    {
        //  ローカル座標で移動(左足)
        leftFoot.transform.DOLocalPath(
            leftMoveWayPoints,          // 通過地点のリスト
            productionTime,             // 演出時間
            movePathType                // 曲線の種類 、Catmull-Rom曲線を使用(WayPoint同士を曲線で繋ぐ)
                                        //gizmoColor: Color.red     // Gizmosで赤色を設定
        )
        .SetOptions(moveOption)         // 「最後のWayPoint→最初のWayPoint」が繋がるようになります
        .SetLoops(-1, moveLoopType)     // 繰り返し（ループ）、移動後、最初の位置から再び移動を開始
        .SetEase(ease)                  // 変化の緩急（＝イージング）を指定、一定の速度で値が変化
        .SetLink(gameObject);           // このGameObjectが削除された自動で止めるように紐付ける

        //  ローカル座標で移動(右足)
        rightFoot.transform.DOLocalPath(
            rightMoveWayPoints,         // 通過地点のリスト
            productionTime,             // 演出時間
            movePathType                // 曲線の種類 、Catmull-Rom曲線を使用(WayPoint同士を曲線で繋ぐ)
                                        //gizmoColor: Color.red     // Gizmosで赤色を設定
        )
        .SetOptions(moveOption)         // 「最後のWayPoint→最初のWayPoint」が繋がるようになります
        .SetLoops(-1, moveLoopType)     // 繰り返し（ループ）、移動後、最初の位置から再び移動を開始
        .SetEase(ease)                  // 変化の緩急（＝イージング）を指定、一定の速度で値が変化
        .SetLink(gameObject);           // このGameObjectが削除された自動で止めるように紐付ける
    }

}
